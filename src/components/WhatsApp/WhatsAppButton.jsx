...
const WhatsAppButton = () => {
    ...
    const productMatch = location.pathname.match(/^\/productos\/([^/]+)/)
    const productId = productMatch ? productMatch[1] : null
    const productTitle = productId
        ? products.find((item) => String(item.id) === productId)?.title
        : null
    ...
    return (
        <>
            {open ? (
                <WhatsAppChatPanel productTitle={productTitle} onClose={closePanel} />
            ) : null}
            <button
                type="button"
                className="whatsapp-float"
                onClick={togglePanel}
                aria-label={open ? 'Cerrar chat de soporte' : 'Iniciar chat por WhatsApp'}
                aria-expanded={open}
